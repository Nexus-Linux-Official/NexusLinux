...
* `nexus-unstable.sh` fault-tolerant unstable builder added
...
* `archiso/packages.x86_64:160` + `packages_desktop.x86_64:160` → `kde-applications`, `gnome`, `gnome-extra` added (was partial `kate/kcalc/nautilus`)

### Rust Workspace — All 8 Tools Compile
* `rust-workspace/Cargo.toml:10` `colored` workspace dep added, `Cargo.lock` synced
...
* `localpkgs/rust-tools/PKGBUILD:28` `git+tag v0.1.0` → `cp -r …/rust-workspace` local, `--locked/--frozen` removed, `103` `nexus-build-helpers`→`nexus-build` binary map, `depends=(sysinfo nix…)` Rust crates removed
...
* `archiso/pacman.conf:1` absolute `/home/cahit` `[nexus]` → removed (build script dynamically prepends; CI compatible), then re-added in `ccd56f1` for host build + `sudo pacman -Sy`
...
* `localpkgs/rust-workspace/.gitignore:33` `**/src/` was swallowing rust-workspace → `!localpkgs/rust-workspace/**/src/**` exception
...
* `.claude/SKILLS:1` 210 skills (10 categories: archiso 30, calamares 25, rust 30, pkgbuild 25, shell 25, git 15, system 20, qa 15, python 15, meta 10) — `.claude/SKILLS/README.md:1` + `example/example.md:1` template, each skill `<skill>/<skill>.md:1`
* `.Claude/SKILLS:1` copy added (case visibility)
* Python `%7.5` calculation: for `2797`→`10976`, `8179` bytes shell→Python required (`build.sh:2003` + `nexus-unstable.sh:4742` + `gen-nexus-keyring.sh:1546` = `8291`)
* `15` external repos (`Extra`, `Multilib`, `nex`, `nexsolv`, `Nexus-Calamares`, `nexus-fish-config`, `nexus-handheld`, `nexus-hooks`, `nexus-kde-settings`, `nexus-kernel`, `nexus-kernel-manager`, `nexus-keyring`, `Nexus-Settings`, `nexus-wallpapers`, `nexus-zsh-config`) synced
